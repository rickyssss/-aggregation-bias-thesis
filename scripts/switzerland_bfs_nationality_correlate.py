...
# major foreign citizenship groups resident in Switzerland (2024, permanent resident population)
CITZ_LABELS = {
    "8218": "italy",
    "8207": "germany",
    "8231": "portugal",
...
def load_jsonstat(path):
    ...
    def get(geo, citz="-99999"):
        yi = 0
        gi = idx[GEO_CODE][geo]
        bti = 0  # Bevölkerungstyp fixed to '1' (permanent resident population), only value queried
        bpi = 0  # Geburtsort fixed to '-99999' (total), only value queried
        ci = idx["Staatsangehörigkeit"][citz]
        pos = yi * strd[0] + gi * strd[1] + bti * strd[2] + bpi * strd[3] + ci * strd[4]
        return values[pos]
...
idxA, labelsA, getA, geo_codes = load_jsonstat(f"{SC}/result_nationality.json")

VAR_NAMES = [f"{name}_citizens_per1000" for name in CITZ_LABELS.values()]
...
def build_table(geo_set):
    ...
    for gcode in geo_codes:
        ...
        for code, name in CITZ_LABELS.items():
            v = getA(gcode, citz=code)
            rec[f"{name}_citizens_per1000"] = v / total * 1000.0 if v is not None else None
        rows.append(rec)
    return rows
...
write_csv(coarse, "data/Switzerland/switzerland_bfs_nationality_coarse_canton.csv")
write_csv(fine, "data/Switzerland/switzerland_bfs_nationality_fine_commune.csv")
...
with open(f"{SC}/nationality_pairwise.csv", "w", newline="") as f:
