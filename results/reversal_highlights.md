...
## 2026-07-22: 新規12カ国+スペイン(Eurostat人口構造指標) 364組み合わせの内訳（reversed 10件・magnitude_change 110件）

- **reversed（符号逆転）**: 10件 / **magnitude_change（大きさが大きく変化）**: 110件 / **similar（ほぼ同様）**: 244件（合計364件）。国別内訳はオーストリア(28組、reversed 0)・ベルギー(28、0)・ブルガリア(28、0)・チェコ(28、1)・ドイツ(28、1)・ギリシャ(28、0)・クロアチア(28、3)・ハンガリー(28、1)・ルーマニア(28、1)・セルビア(28、0)・スロバキア(28、3)・トルコ(28、0)・スペイン(28、0)。
- **注目例1（クロアチア、総従属人口指数 vs 年少従属人口指数、最も強い逆転）**: NUTS2(県、n=4)単位corr=-0.940(有意でない) → NUTS3(郡、n=21)単位corr=+0.113(有意でない)。粗い単位のn=4という極端な少なさで、たまたま非常に強い負の相関に見えていた典型例。有意性検定では両方とも「有意でない」と出ており、n=4での強い相関係数を鵜呑みにする危険性を示す好例。
- **注目例2（ドイツ、年少従属人口指数 vs 人口の年齢中央値）**: NUTS2(州、n=38)単位corr=-0.192(有意でない) → NUTS3(郡、n=400)単位corr=+0.060(有意でない)。地域数の多いドイツでも符号が反転する組み合わせが見つかったが、どちらも弱く有意ではないため実質的にはほぼ無関係。
- **注目例3（スロバキア、総従属人口指数 vs 老年従属人口指数）**: NUTS2(県、n=4)単位corr=-0.166(有意でない) → NUTS3(郡、n=8)単位corr=+0.100(有意でない)。スロバキアはn=4の粗い単位で3件の符号逆転が見つかったが、いずれも|corr|が小さく、偶然のばらつきの可能性が高い。
- **示唆**: 地域数が極端に少ない国(クロアチア・セルビア・スロバキア、いずれもNUTS2はn=4)ほど、粗い集計単位での相関係数が偶然大きくぶれやすく、符号逆転も起きやすいことが13カ国横断で確認できた。一方でドイツのように地域数が多い国でも、相関が弱い(ほぼ無関係な)組み合わせでは符号が入れ替わることがあり、これは「意味のある逆転」というより「ゼロ近辺でのノイズ」と解釈すべき点に注意。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=5708〜6071）に記録済み。詳細は国ごとに`results/summaries/id5708_5735_austria_eurostat_demography_pairwise.csv`〜`id6044_6071_spain_eurostat_demography_pairwise.csv`（13ファイル）、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: GDP×人口構造(ベルギー・ブルガリア・チェコ・クロアチア・ハンガリー・ルーマニア・スロバキア・トルコ) 64組み合わせの内訳（reversed 3件・magnitude_change 44件）

- **reversed（符号逆転）**: 3件 / **magnitude_change（大きさが大きく変化）**: 44件 / **similar（ほぼ同様）**: 17件（合計64件）。同日1回目の人口構造どうしの組み合わせ(reversed比率2.7%)より、GDP×人口構造の組み合わせ(reversed比率4.7%)の方が符号逆転の割合が高かった。
- **注目例1（クロアチア、1人あたりGDP vs 年少従属人口指数、最も大きな変化）**: NUTS2(県、n=4)単位corr=+0.816(有意でない) → NUTS3(郡、n=21)単位corr=-0.093(有意でない)。粗い単位での「GDPが高い地域ほど子供が多い」という一見意外な強い相関が、細かい単位で見るとほぼ消えて符号も反転する。n=4の少なさが偶然の強い相関を生んだ可能性が高い。
- **注目例2（ベルギー、1人あたりGDP vs 80歳以上人口割合）**: NUTS2(地域、n=11)単位corr=-0.074(有意でない) → NUTS3(郡、n=44)単位corr=+0.269(有意でない)。都市部と郊外の違いが、より細かい単位でGDPと高齢化率の関係の見え方を変えた可能性がある例。
- **注目例3（ブルガリア、1人あたりGDP vs 男性1000人あたり女性数）**: 州単位corr=-0.057(有意でない) → 市郡単位corr=+0.21(有意でない)。
- **示唆**: 経済指標と人口構造指標という定義上直接つながっていない変数どうしの組み合わせでは、定義上関連が強い変数どうし(人口構造指標どうし)の組み合わせに比べて、集計単位を変えたときの符号逆転が起きやすいことが確認できた。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6072〜6135）に記録済み。詳細は国ごとに`results/summaries/id6072_6079_belgium_gdp_demography_pairwise.csv`〜`id6128_6135_turkey_gdp_demography_pairwise.csv`（8ファイル）、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: GDP×人口構造(オーストリア・ドイツ・ギリシャ・セルビア・スペイン・イタリア) 48組み合わせの内訳（reversed 5件・magnitude_change 29件）

- **reversed（符号逆転）**: 5件 / **magnitude_change（大きさが大きく変化）**: 29件 / **similar（ほぼ同様）**: 14件（合計48件）。国別内訳はオーストリア(8組、reversed 1)・ドイツ(8、0)・ギリシャ(8、0)・セルビア(8、3)・スペイン(8、1)・イタリア(8、0)。
- **注目例1（セルビア、1人あたりGDP vs 総従属人口指数、最も強い逆転）**: NUTS2(広域圏、n=4)単位corr=-0.980(有意) → NUTS3(地区、n=25)単位corr=+0.066(有意でない)。粗い単位のn=4という極端な少なさで非常に強い負の相関に見えていたが、細かい単位ではほぼ消滅し符号も逆転。
- **注目例2（セルビア、1人あたりGDP vs 老年従属人口指数）**: NUTS2単位corr=-0.966(有意) → NUTS3単位corr=+0.054(有意でない)。セルビアはNUTS2がn=4の8組中3組で符号逆転が起きており、地域数の少なさが集計単位間の結果の不安定さに直結することを改めて示す例。
- **注目例3（スペイン、1人あたりGDP vs 年少従属人口指数）**: NUTS2(自治州、n=19)単位corr=-0.263(有意でない) → NUTS3(県、n=59)単位corr=+0.081(有意でない)。どちらも弱く有意ではなく、「ほぼ無関係な変数どうしで符号だけ入れ替わった」ノイズ的な逆転。
- **示唆**: 今回もセルビアのように地域数が極端に少ない国(NUTS2 n=4)ほど、粗い集計単位での相関が偶然大きくぶれやすく符号逆転が起きやすいことが確認できた。一方、地域数の多いドイツ(NUTS2 n=38)・イタリア(n=21)では逆転はゼロで、大きさの変化(magnitude_change)にとどまった。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6136〜6183）に記録済み。詳細は国ごとに`results/summaries/id6136_6143_austria_gdp_demography_pairwise.csv`〜`id6176_6183_italy_gdp_demography_pairwise.csv`（6ファイル）、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: 観光集中度×人口構造(13カ国) 104組み合わせの内訳（reversed 11件・magnitude_change 77件）— イタリアで逆転率6/8と突出

- **reversed（符号逆転）**: 11件 / **magnitude_change（大きさが大きく変化）**: 77件 / **similar（ほぼ同様）**: 16件（合計104件）。国別内訳: オーストリア(8組、reversed 1)・ベルギー(8、0)・ブルガリア(8、2)・チェコ(8、0)・クロアチア(8、0)・ドイツ(8、0)・ギリシャ(8、0)・ハンガリー(8、1)・ルーマニア(8、0)・セルビア(8、0)・スロバキア(8、1)・スペイン(8、0)・**イタリア(8、6)**。
- **最も注目すべき例（イタリア、8組中6組が符号逆転）**: 「観光集中度(延べ宿泊者数/人口)」と「老年従属人口指数」「65歳以上人口割合」「性比」「年齢中央値」「総従属人口指数」「80歳以上人口割合」の6変数すべてで、州(regione、n=21)単位と県(provincia、n=107)単位の相関の符号が逆転した。例えば老年従属人口指数はcorr=-0.378(州)→+0.081(県)。いずれも有意性検定では両方とも「有意でない」ため統計的にはノイズの範囲内だが、これほど高い割合(6/8)で逆転が同時多発したのは全データセット中でも珍しい。
- **注目例2（ブルガリア、観光集中度 vs 年少従属人口指数）**: 州(オブラスト、n=6)単位corr=+0.806(有意でない) → 市郡(オプシュティナ、n=28)単位corr=-0.050(有意でない)。粗い単位のn=6という少なさが見かけ上の強い相関を生んだ可能性が高い。
- **示唆**: 人口構造と定義上まったく独立な観光統計を組み合わせたことで、これまでで最も高い逆転率(11/104=10.6%)が確認できた。特にイタリアの逆転集中は、州単位での観光地偏在(海岸部・北部大都市など)が、より細かい県単位で見ると地域内のばらつきによって関係性自体が反転しうることを示す好例。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6184〜6287）に記録済み。詳細は国ごとに`results/summaries/id6184_6191_austria_tourism_demography_pairwise.csv`〜`id6280_6287_italy_tourism_demography_pairwise.csv`（13ファイル）、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: 就業率×人口構造(13カ国) 104組み合わせの内訳（reversed 18件・magnitude_change 68件）— セルビアは8組全てが符号逆転

- **reversed（符号逆転）**: 18件 / **magnitude_change（大きさが大きく変化）**: 68件 / **similar（ほぼ同様）**: 18件（合計104件）。国別内訳: オーストリア(8組、reversed 1)・ベルギー(8、1)・ブルガリア(8、0)・チェコ(8、2)・クロアチア(8、1)・ドイツ(8、3)・ギリシャ(8、0)・ハンガリー(8、0)・ルーマニア(8、0)・**セルビア(8、8=全件)**・スロバキア(8、0)・スペイン(8、2)・イタリア(8、0)。
- **最も突出した例（セルビア、8組すべてが符号逆転）**: 「就業率」と人口構造8変数すべての組み合わせで、広域圏(NUTS2、n=4)単位と地区(NUTS3、n=25)単位の相関の符号が逆転した。特に「就業率 vs 男性1000人あたり女性数」は広域圏単位でcorr=+0.972と統計的に**有意**なほど強い正の相関だったが、地区単位ではcorr=-0.126(有意でない)と符号ごと反転した。セルビアは前回のGDP×人口構造(id=6256〜6263)でも8組中3組が逆転しており、就業率では全件逆転とさらに極端化した。同じ4つの広域圏という極端に粗い区分を使い続ける限り、変数を変えても逆転が繰り返し起こりやすいことを示す最重要事例。
- **注目例2（ドイツ、就業率 vs 0-14歳人口割合）**: 州(NUTS2、n=38)単位corr=+0.403(有意でない) → 郡(NUTS3、n=400)単位corr=-0.207(有意でない)。地域数の多い国でも符号逆転は起こりうることを示す例(ドイツは8組中3組で逆転)。
- **示唆**: セルビアの「4つの広域圏」という粗すぎる区分が、変数によらず繰り返し不安定な(そして時に有意な)相関を生み出すことが、GDP・観光・就業率という3種類の独立変数で一貫して確認された。集計単位の粗さが極端な場合の危険性を示す最も説得力のある事例として、発表資料での活用を推奨する。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6288〜6391）に記録済み。詳細は国ごとに`results/summaries/id6288_6295_austria_employment_demography_pairwise.csv`〜`id6384_6391_italy_employment_demography_pairwise.csv`（13ファイル）、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: スイス国籍構成データ新規4か国籍(トルコ・セルビア・北マケドニア・スリランカ) 30組み合わせの内訳（reversed 4件・magnitude_change 16件・similar 10件）— 出身地域が異なる移民集団どうしで逆転集中

- **reversed（符号逆転）**: 4件 / **magnitude_change（大きさが大きく変化）**: 16件 / **similar（ほぼ同様）**: 10件（合計30件）。
- **注目した例**: 「スペイン国籍者比率」と「北マケドニア/セルビア/スリランカ国籍者比率」の3組すべてで、canton(州、n=26)単位では負の相関(有意でない)だったのが、commune(市区町村、n=2131)単位では正の相関(有意)に逆転した。例えば「スペイン国籍者比率 vs 北マケドニア国籍者比率」はcorr=-0.173(canton、有意でない)→corr=+0.096(commune、有意)。「イタリア国籍者比率 vs スリランカ国籍者比率」もcorr=-0.181(canton、有意でない)→corr=+0.086(commune、有意)と同様のパターン。
- **対照的に頑健だった例**: 新規4か国籍(トルコ・セルビア・北マケドニア・スリランカ)同士の相関6組はすべてsimilarで、いずれも州単位・市区町村単位の両方で有意な正の相関(corr=0.4〜0.6程度)を保った。
- **示唆**: 出身地域が離れた移民集団どうし(スペイン系 vs バルカン半島・南アジア系)の相関は、州単位では「異なる州に集中する」ため見かけ上マイナスに見えるが、市区町村単位まで細かく見ると「移民が多い都市部ほど出身国を問わず外国籍者比率が高い」という素直な正の関係に変わる、という解釈ができる。一方、出身地域が近い移民集団どうしの相関は集計単位を変えても頑健。集計単位の効果が変数の組み合わせ(ここでは移民の出身地域の近さ)に強く依存することを示す好例。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6392〜6421）に記録済み。詳細は`results/summaries/id6392_6421_switzerland_nationality2_pairwise.csv`、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: アイルランドCSO SAPS(出生地・宗教・民族的背景・アイルランド語) 66組み合わせの内訳（reversed 6件・magnitude_change 27件・similar 33件）— 両方の集計単位で有意なまま逆転する例あり

- **reversed（符号逆転）**: 6件 / **magnitude_change（大きさが大きく変化）**: 27件 / **similar（ほぼ同様）**: 33件（合計66件）。アイルランドCSO PxStat APIが繰り返しHTTP 500で接続不可だった問題を、CSOが直接配布しているSAPS(Small Area Population Statistics)のダウンロード用CSV(county 31単位・Electoral Division 3420単位、登録不要)に切り替えて解決した。
- **注目例1（イギリス出生比率 vs その他の白人比率、最も注目すべき逆転）**: county(n=31)単位corr=-0.366(**有意**) → Electoral Division(n=3420)単位corr=+0.119(**有意**)。粗い単位・細かい単位の両方で統計的に有意なまま符号が逆転する珍しいケースで、「集計単位を変えると統計的に自信を持って反対の結論を導いてしまう」という集計バイアスの危険性を最も強く示す事例。
- **注目例2（ポーランド出生比率 vs インド出生比率）**: county単位corr=-0.066(有意でない、ほぼ無相関) → Electoral Division単位corr=+0.397(有意)。粗い単位では検出できなかった「移民の集住地区」という現象が、細かい単位で初めて統計的に有意な形で見えるようになった例。
- **注目例3（ホワイト・アイリッシュ・トラベラー比率 vs アジア系またはアジア系アイルランド人比率）**: county単位corr=-0.130(有意でない) → Electoral Division単位corr=+0.119(有意)。
- **示唆**: 「移民の出身地」「宗教」「民族的背景」「言語」という互いに独立に定義された属性の組み合わせでも、9.1%(6/66)で符号逆転が起きた。特に例1のように両方の単位で有意なまま逆転する例は、これまでのEurostatデータ(粗い単位側だけが有意なパターンが多かった)と対照的で、発表資料の目玉事例として使える。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6422〜6487）に記録済み。詳細は`results/summaries/id6422_6487_ireland_saps_pairwise.csv`、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: アイルランドCSO SAPS 第2弾(世帯タイプ・住宅設備・労働力状態・社会階層・教育) 253組み合わせの内訳（reversed 31件・magnitude_change 156件・similar 66件）— 「田舎らしさ」を表す指標群で逆転が集中

- **reversed（符号逆転）**: 31件(12.3%) / **magnitude_change（大きさが大きく変化）**: 156件 / **similar（ほぼ同様）**: 66件（合計253件）。前回(id=6422〜6487、出生地・宗教・民族的背景・アイルランド語)とは別のSAPSテーマ(世帯タイプ・住宅の占有形態や設備・労働力状態・社会階層・教育)を追加抽出した。
- **注目した例(「田舎らしさ」の指標どうしで一貫して逆転)**: 石油暖房比率・浄化槽比率・私設水源比率(いずれも都市ガスや公共上下水道が来ていない農村部で高くなる指標)は、county(n=31)単位では「傷病により就労不能な人の比率」「非熟練労働者比率」と**有意な正の相関**(corr=+0.32〜+0.51)を示すのに、Electoral Division(n=3420、より細かい地区)単位では同じ組み合わせがすべて**有意な負の相関**(corr=-0.10〜-0.27)に逆転した。具体的には「石油暖房比率 vs 就業率」がcounty単位corr=-0.605(有意)→ED単位corr=+0.062(有意)、「浄化槽比率 vs 傷病就労不能比率」がcounty単位corr=+0.442(有意)→ED単位corr=-0.274(有意)など、13組で両方の集計単位が有意なまま逆転した。
- **考えられる解釈**: county単位では「農村県ほど高齢化・産業構造の面で不利」という粗い地域差(例えば伝統的に農業や離島が多い県)が前面に出るが、Electoral Division単位まで細かく見ると、同じ「農村的」な地区の中にも富裕な郊外(私設水源や浄化槽を持つ新興住宅地など)と困窮した地区が混在しており、県単位の平均では覆い隠されていた多様性が見えるようになるためと考えられる。
- **統計的な注意点**: ED単位はn=3420と非常に大きいため、corr=0.06程度のごく小さな相関でも「統計的に有意」と判定される(自由度が大きいほど臨界値が小さくなるため)。「有意=実質的に意味のある大きさ」ではない点は、発表で強調しておくべき注意点。
- **他の顕著な逆転**: 「1人暮らし世帯比率」は石油暖房・都市ガス暖房・浄化槽の3つすべてと逆転関係にあり(county単位では農村的インフラと正相関、ED単位では負相関)、「持ち家(ローン無し)比率 vs 傷病就労不能比率」もcounty単位corr=+0.394(有意)→ED単位corr=-0.169(有意)と逆転した。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6488〜6740）に記録済み。詳細は`results/summaries/id6488_6740_ireland_saps_theme2_pairwise.csv`、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: アイルランドCSO SAPS 第3弾(通勤手段・健康・産業構造・自動車保有と通信環境) 210組み合わせの内訳（reversed 19件・magnitude_change 158件・similar 33件）— 「在宅勤務」「短時間通勤」が逆転の中心

- **reversed（符号逆転）**: 19件(9.0%) / **magnitude_change（大きさが大きく変化）**: 158件 / **similar（ほぼ同様）**: 33件（合計210件）。
- **最も顕著な例(通勤時間15分未満比率 vs ブロードバンド普及率)**: county(n=31)単位ではcorr=-0.760(**強い負の相関、有意**)、つまり「通勤が短い県ほどブロードバンドが普及していない」ように見えるが、Electoral Division(n=3420)単位ではcorr=+0.081(有意)とほぼ無相関に近い正の値に変わった。county単位での強い負の相関は、実際には「首都ダブリン近郊(通勤時間は長いがブロードバンドは普及)」対「地方の農村県(通勤は短いがブロードバンドが手薄)」という31県だけの粗い対比を反映したもので、地区単位まで見ると本来ほぼ無関係であることがわかる、集計単位の危うさを象徴する事例。
- **在宅勤務比率が絡む逆転(8件)**: 「在宅勤務比率」は「徒歩/自転車通勤比率」「通勤時間1時間以上比率」「障害者比率」「介護者比率」「農林水産業比率」「専門サービス業比率」「車なし世帯比率」「複数台保有世帯比率」の8変数すべてで符号逆転を起こした。特に「在宅勤務比率 vs 農林水産業比率」はcounty単位corr=-0.607(有意)→ED単位corr=+0.303(有意)で、両方の単位で有意なまま逆転。county単位では「農業が盛んな県ほど在宅勤務が少ない」ように見えるが、実際には農家自体が「自宅(兼職場)で働く」ため地区単位では正の関係になる、という定義上の交絡が示唆される。
- **示唆**: 通勤・在宅勤務に関する変数は、county単位の少数派(n=31)のサンプルでは「都市 vs 地方」という一次元の対比に押し込められてしまうため、他の変数との相関が実態と異なる方向に出やすいことが今回のデータから見えました。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6741〜6950）に記録済み。詳細は`results/summaries/id6741_6950_ireland_saps_theme3_pairwise.csv`、経緯は`results/aggregation_effect_log.md`の該当項を参照。

## 2026-07-22: アイルランドCSO SAPS 第4弾(職業の社会階層分類) 36組み合わせの内訳（reversed 1件・magnitude_change 31件・similar 4件）— 「技能職」と「単純作業職」の地理的すみ分け

- **reversed（符号逆転）**: 1件(2.8%) / **magnitude_change（大きさが大きく変化）**: 31件 / **similar（ほぼ同様）**: 4件（合計36件）。
- **唯一の逆転例(技能職比率 vs 単純作業職比率)**: county(n=31)単位ではcorr=+0.445(有意)、つまり「ブルーカラー職(技能職・単純作業職)が多い県はどちらも多い」ように見えるが、Electoral Division(n=3420)単位ではcorr=-0.113(有意)に逆転した。地区単位まで見ると、技能職(建設・製造の現場職人)が集中する地区と単純作業職(倉庫・清掃・小売の補助業務など)が集中する地区は実際には別々の場所に分かれており、県単位の平均ではこの地理的なすみ分けが見えなくなっていたと考えられる。
- `similar`と判定された全件も含め`results/summary_table.csv`（id=6951〜6986）に記録済み。詳細は`results/summaries/id6951_6986_ireland_saps_theme4_pairwise.csv`、経緯は`results/aggregation_effect_log.md`の該当項を参照。
